...
class PushBtnTouchListener implements View.OnTouchListener {
	...
	private View mView, mCancelView;
	private Point mViewCenter;
	private static final double PI = 3.14159265359;

	public PushBtnTouchListener(View mView, View mCancelView) {
		this.mView = mView;
	...
	private FrameLayout.LayoutParams cancelBtnLP;
	float lastX = -1;
	float lastY = -1;
	...
	public boolean onTouch(View pushView, MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			cancelImgWidth = cancelBtnLP.width;
			cancelImgHeight = cancelBtnLP.height;
			lastX = event.getRawX();
			lastY = event.getRawY();
			refreshImageCenter();
			break;
		...
		case MotionEvent.ACTION_MOVE:
			...
			Point imageRB2 = new Point(mView.getLeft(), mView.getTop());

			Point anglePoint = getAnglePoint(O, imageRB, angle);
			Point anglePoint2 = getAnglePoint(O, imageRB2, 90 + angle);
			...
			/*
			 * cancelBtnLP.leftMargin = (int) (anglePoint.x - imgLP.width -
			 * cancelImgWidth / 2); cancelBtnLP.topMargin = (int) (anglePoint.y
			 * - imgLP.height - cancelImgHeight / 2);
			 */
			cancelBtnLP.leftMargin = (int) (anglePoint2.x - cancelImgWidth / 2);
			cancelBtnLP.topMargin = (int) (anglePoint2.y - cancelImgHeight / 2);
